...
#  network = data.google_compute_network.main.self_link #  == "" ? var  : data.google_compute_network.main.self_link
  region  = var.google_region
}
...
resource "google_compute_router" "aws1" {
  ...
  region  = var.google_region
  network = var.google_network_name
  #network = data.google_compute_network.main.name == "" ? "default_netowork" : data.google_compute_network.main.name
...
resource "google_compute_router" "aws2" {
  ...
  region  = var.google_region
  network = var.google_network_name
  #network = data.google_compute_network.main.name == "" ? "default_network" : data.google_compute_network.main.name
